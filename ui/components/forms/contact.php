...
<form novalidate class="space-y-2" x-data="formContact($el)" data-action="<?php echo $action; ?>" data-action-url="<?php echo $action_url; ?>"
  ...
  <div>
    <input type="tel" placeholder="Telephone *" class="w-full border px-5 py-2" x-model="fields.telephone"
      :class="states?.fields?.telephone?.error && 'border-red-500'" @keyup="onValidate" :disabled="loading"
      x-mask="9999999999" />
    <p class="text-red-500" x-cloak x-show="states?.fields?.telephone?.error" x-text="states?.fields?.telephone?.error">
    </p>
  </div>
  <div>
    <textarea placeholder="Message" class="w-full border px-5 py-2 min-h-[120px]" x-model="fields.message"
